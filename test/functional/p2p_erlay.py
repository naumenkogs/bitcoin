...
from test_framework.p2p import P2PDataStore, p2p_lock, MIN_P2P_VERSION_SUPPORTED
...
class TestP2PConn(P2PDataStore):
    def __init__(self, recon_version, local_salt, be_recon_requestor=False, be_recon_responder=False):
        super().__init__()
...
class ReconciliationTest(BitcoinTestFramework):
    ...
    def test_incoming_recon(self):
        ...
        self.test_node0 = self.nodes[0].add_p2p_connection(TestP2PConn(recon_version=0, local_salt=LOCAL_SALT))
        self.test_node0.wait_for_verack()

        self.test_node = self.nodes[0].add_p2p_connection(TestP2PConn(recon_version=1,
            local_salt=LOCAL_SALT, be_recon_requestor=True, be_recon_responder=False))
    ...
    def test_outgoing_recon(self):
        ...
        for i in range(MAX_OUTBOUND_FLOOD_TO):
            flooding_node = self.nodes[0].add_p2p_connection(TestP2PConn(
                recon_version=0, local_salt=LOCAL_SALT), node_outgoing=True)
            flooding_node.wait_for_verack()

        self.test_node = self.nodes[0].add_p2p_connection(TestP2PConn(recon_version=1,
            local_salt=LOCAL_SALT, be_recon_requestor=False, be_recon_responder=True), node_outgoing=True)
        self.test_node.wait_for_verack()
