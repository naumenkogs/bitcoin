...
BOOST_AUTO_TEST_CASE(InitiateReconciliationRequest)
{
    ...
    tracker.AddToSet(peer_id0, Wtxid::FromUint256(frc.rand256()));
    tracker.AddToSet(peer_id0, Wtxid::FromUint256(frc.rand256()));
    // Still empty because not enough time passed even though we have txs in sets.
    ...
    BOOST_CHECK_EQUAL(local_set_size2, 0);
    ...
    tracker.AddToSet(peer_id0, Wtxid::FromUint256(frc.rand256()));
    tracker.AddToSet(peer_id0, Wtxid::FromUint256(frc.rand256()));
    tracker.ReadyDelayedTransactions(peer_id0);
    const auto reconciliation_request_params3 = tracker.InitiateReconciliationRequest(peer_id0);
    BOOST_CHECK(reconciliation_request_params3 != std::nullopt);
    const auto [local_set_size3, local_q_formatted3] = (*reconciliation_request_params3);
    BOOST_CHECK_EQUAL(local_set_size3, 3);
    BOOST_CHECK_EQUAL(local_q_formatted3, uint16_t(32767 * 0.25));
}
