...
size_t PeerManagerImpl::GetFloodingOutboundsCount(const CNode& skip_node) const
{
    ...
    m_connman.ForEachNode([this, &result, &skip_node](const CNode* pnode) {
        ...
        const auto recon_peer_flood_to = m_reconciliation.IsPeerChosenForFlooding(pnode->GetId());
        if (recon_peer_flood_to) {
            // Nodes supporting reconciliation still may be meant for flooding.
            if (*recon_peer_flood_to) ++result;
        } else {
                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::VERSION) {
        ...
        if (greatest_common_version >= WTXID_RELAY_VERSION) {
            ...
            if (pfrom.MightSupportTransactionRelay()) {
                const auto [be_recon_requestor, be_recon_responder, recon_version, recon_salt] =
                ...
                m_connman.PushMessage(&pfrom, msg_maker.Make(NetMsgType::SENDRECON,
                    be_recon_requestor, be_recon_responder, recon_version, recon_salt));
            }
        }
    ...
    if (msg_type == NetMsgType::SENDRECON) {
        ...
        const size_t outbound_flooders = GetFloodingOutboundsCount(pfrom);
    ...
    if (msg_type == NetMsgType::TX) {
        ...
        if (result.m_result_type == MempoolAcceptResult::ResultType::VALID) {
            ...
            const auto recon_peer_responder = m_reconciliation.IsPeerResponder(pfrom.GetId());
            bool flood = !recon_peer_responder || !(*recon_peer_responder);
...
bool PeerManagerImpl::SendMessages(CNode* pto)
{
...

    {
        ...
        std::vector<CInv> vInv;
        {
            ...
            const auto recon_is_for_flooding = m_reconciliation.IsPeerChosenForFlooding(pto->GetId());
            ...
            if (pto->m_tx_relay != nullptr) {
                ...
                if (pto->m_tx_relay->nNextInvSend < current_time) {
                    fSendTrickle = true;
                    if (!recon_is_for_flooding && pto->IsInboundConn()) {
                ...
                if (fSendTrickle) {
                    ...
                    while (!vInvTx.empty() && nRelayedTransactions < INVENTORY_BROADCAST_MAX) {
                        ...
                        const auto recon_set_size = m_reconciliation.GetPeerSetSize(pto->GetId());
                        ...
                        if (recon_set_size) {
                            ...
                            if (txs_to_reconcile.size() + *recon_set_size < MAX_PEER_TX_ANNOUNCEMENTS) {
                                ...
                                if (!(*recon_is_for_flooding) || !flood_tx) {
