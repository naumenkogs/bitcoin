    ...
    for(const auto& [peer_id, peer] : m_peer_map) {
        ...
        if (fanout) {
            ...
            if (!result.m_succeeded) {
                ...
                // to its set, in which case the two transactions mapped to the same
                // short id won't be announced between these two peers.
                // Descendants are also announced in this case because otherwise they may
                // get reconciled before the INV containing the parent is sent, in which
                // case they will be treated as orphans.
                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::RECONCILDIFF) {
        ...
        if (valid_finalization) {
            ...
            LogDebug(BCLog::NET, "reconcildiff from peer=%d violates reconciliation protocol; disconnecting\n", pfrom.GetId());
            pfrom.fDisconnect = true;
            return;
        }
        return;
