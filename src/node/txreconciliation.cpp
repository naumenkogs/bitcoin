...
private:
                               ...
                               std::vector<uint32_t>& txs_to_request, std::vector<uint256>& txs_to_announce, std::optional<bool>& result)
    {
        ...
        if (local_sketch.Merge(remote_sketch).Decode(differences)) {
            ...
            result = true;
            LogPrintLevel(BCLog::TXRECONCILIATION, BCLog::Level::Debug, "Reconciliation we initiated with peer=%d has succeeded at extension step, "
                                 "request %i txs, announce %i txs.\n",
                     peer_id, txs_to_request.size(), txs_to_announce.size());
        } else {
            ...
            result = false;
            LogPrintLevel(BCLog::TXRECONCILIATION, BCLog::Level::Debug, "Reconciliation we initiated with peer=%d has failed at extension step, "
...
public:
    ...
    void HandleExtensionRequest(NodeId peer_id) EXCLUSIVE_LOCKS_REQUIRED(!m_txreconciliation_mutex)
    {
        ...
        if (recon_state.m_capacity_snapshot == 0) {
            ...
            // request extension.
            LogPrintLevel(BCLog::TXRECONCILIATION, BCLog::Level::Debug, "Peer=%d violated the protocol by requesting an extension "
